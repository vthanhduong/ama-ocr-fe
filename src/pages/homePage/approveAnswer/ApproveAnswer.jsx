...
export default function ApproveAnswer() {
    ...
    let token = getToken();
    useEffect(() => {
        qaApi.getUnapprovedAnswers(token).then((res) => {
    ...
    const handleAprroveAnswer = async (id,status) => {
        setLoading(true);
        let data = {
            id,
            status
        ...
        try {
            const res = await qaApi.approveAnswer(data, token);
            setReload(Date.now())
        } catch (error) {
                ...
                <tbody>
                    <tr>
                        ...
                        <td className="p-2 border">
                            <LoadingButton
                                onClick={() => handleAprroveAnswer(item.id, 2)}
                                loading={loading}
                            ...
                            <LoadingButton
                                onClick={() => handleAprroveAnswer(item.id, 3)}
                                loading={loading}
                                className="w-full bg-red-500 mt-2 text-white cursor-pointer"
                            >
                                Không Duyệt
                            </LoadingButton>
                        </td>
    ...
    return (
        <div className='w-full mt-10 bg-white overflow-y-auto px-2'>
            {renderContent()}
        </div>
  )
}
